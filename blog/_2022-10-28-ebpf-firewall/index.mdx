...
eBPF has its own ISA and there is a runtime for said ISA in the kernel(in eBPF the implementation is JIT'd), the instructions allows calling into parts of the kernel that we might need and gives us the neccesary context to process a given packet.

Furthermore, to prevent crashes in the kernel and hogging out resources before an eBPF program gets loaded into the kernel it goes through a verification process, this verification imposes a lot of limitations in the program as to prevent infinite loops and craches, i.e., it sets an upper-bound for the loop (or strictly prohibits them in older kernel versions) and in the number of instructions, also keeps tracks of read/writes to know that you are using valid memory locations.

If you want to know more about the internals of eBPF [these are the official docs](https://www.kernel.org/doc/Documentation/networking/filter.txt) however I really recommend taking a look at [Cillium's doc on the topic](https://docs.cilium.io/en/stable/bpf/#bpf-guide) they go into much more detail with a lot of clarity.
...
- Possibly a sender's `user_id`
- Destination CIDR
- Possibly a port-range along with possibly a protocol(TCP/UDP)

We took an additional simplification to make the implementation easier(we will see why further along), you can configure the firewall to allow all or deny all, then each rule would just toggle that, so a firewall that denies all traffic with a rule for destination `10.20.30.40`, would deny all traffic from all sources to all destination except for traffic for `10.20.30.40` from any destination.
...
Normally, the way an aya program is structure, we have a user-space crate that loads and is a proxy between the eBPF program and the user and an ebpf crate which has more limitations(such as being `no-std`).
...
The `user_id` is an arbitrary name, the idea is that a single rule can associate multiple source ips, with a CIDR and optionally multiple port ranges.

Rules need to be stored in eBPF's maps, that way we can update the maps during runtime to be able to update the rules.
...
It's more interesting how we handle that after.

### `RULE_MAP`

[Tries](https://en.wikipedia.org/wiki/Trie) are prefix-matching trees, they let us efficiently store and lookup ranges grouped by prefix. For ips this is exactly how CIDR works.
...
Both nodes can be store but the `lookup` method in the `trie` only allows us to find the node with the most specific match. so `10.5.8.170` would match with `10.5.8.128/25` and `10.5.8.110` would match with `10.5.8.0/24`.

But in this case the CIDR are the keys that we can lookup, the values are the port ranges, how we store them is an interesting part of the firewall story, so we will talk about them in more detail later.

This structure, still doesn't allow us to distinguish TCP and UDP and user ids.

To solve this the key for the trie is a little more complicated, we prefix the ip with the user id and the protocol. So a key for TCP(6 is the protocol number) ranges with id 10 for ip `10.5.8.0/24` looks like `10.6.10.5.8.0` and the prefix is 32 (bits of id) + 8 (bits of protocol number) + 24 (bits of /24 CIDR) = 64.

For sources with no id we use `0` as the id. For non-TCP/UDP packets, we store the rule in both the TCP and UDP key and look up TCP(arbitrarily) but only match for port 0 (which we use to encode all ports).
